...
namespace OpenDash.WheelOverlay
{
    public partial class SettingsWindow : Window
    {
        ...
        private void OnBrowseClick(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (dialog.ShowDialog() == true)
                {
                    ...
                    {
                        LogService.Info($"Selected file does not exist: {selectedPath}");
                        System.Windows.MessageBox.Show(
                    ...
                    {
                        LogService.Info($"Selected file is not an executable: {selectedPath}");
                        System.Windows.MessageBox.Show(
                    ...
                    if (_viewModel?.SelectedProfile != null)
                    {
                        ...
                        LogService.Info($"Target executable set to: {selectedPath}");
                    }
                }
            }
            catch (Exception ex)
            {
                LogService.Error("Error selecting target executable", ex);
                System.Windows.MessageBox.Show(
        ...
        private void OnClearClick(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_viewModel?.SelectedProfile != null)
                {
                    ...
                    LogService.Info("Target executable cleared - overlay will be always visible");
                }
            }
            catch (Exception ex)
            {
                LogService.Error("Error clearing target executable", ex);
                System.Windows.MessageBox.Show(
        ...
        private void UpdateTargetDisplay(string? path)
        {
            try
            {
                ...
                else
                {
                    ...
                    catch (ArgumentException)
                    {
                        ...
                        LogService.Info($"Path contains invalid characters: {path}");
                    }
            ...
            {
                LogService.Error("Error updating target display", ex);
