# Developer Quickstart: Discord Chat Overlay

**Branch**: `discord-chat-overlay/v0.1.0`

This guide gets a developer building and running `DiscordChatOverlay` locally. It assumes familiarity with the monorepo and WheelOverlay patterns.
...
This builds `OverlayCore`, `DiscordChatOverlay`, and all test projects. The `DiscordChatOverlay` app is in `src/DiscordChatOverlay/`.
...
In `src/DiscordChatOverlay/Services/DiscordIpcClient.cs`, find:
...
1. Build and run `DiscordChatOverlay` (Debug or Release).
...
4. The user approves. `DiscordChatOverlay` exchanges the code for tokens (PKCE, no client_secret) and stores them in Windows Credential Manager under `"DiscordChatOverlay"`.
...
Settings are saved to `%APPDATA%\DiscordChatOverlay\settings.json`.
Member aliases are saved to `%APPDATA%\DiscordChatOverlay\aliases.json`.
...
| File | Purpose |
|------|---------|
| `src/DiscordChatOverlay/Services/DiscordIpcClient.cs` | Named-pipe transport, IPC framing, HANDSHAKE/AUTH |
| `src/DiscordChatOverlay/Services/VoiceSessionService.cs` | Speaker state machine, debounce, grace period |
| `src/DiscordChatOverlay/Services/AliasService.cs` | aliases.json CRUD, name resolution |
| `src/DiscordChatOverlay/Services/TokenStorageService.cs` | Credential Manager read/write/delete, PKCE helpers |
| `src/DiscordChatOverlay/Models/AppSettings.cs` | settings.json load/save |
| `src/DiscordChatOverlay/ViewModels/OverlayViewModel.cs` | Drives the overlay UI (ActiveSpeakers, OverflowCount, ConnectionIndicator) |
| `src/DiscordChatOverlay/MainWindow.xaml` | Overlay window (always-on-top, click-through, two-column layout) |
...
```bash
powershell -File scripts/discord-chat-overlay/build_msi.ps1
```

Output: `installers/discord-chat-overlay/bin/DiscordChatOverlay-v0.1.0.msi`
...
The app logs all IPC traffic at Debug level to `%APPDATA%\DiscordChatOverlay\logs.txt`. To enable verbose logging:
...
To inspect stored credentials: open **Windows Credential Manager** → **Windows Credentials** → look for `"DiscordChatOverlay"`.
