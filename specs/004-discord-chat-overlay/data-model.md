# Data Model: Discord Chat Overlay
...
### `AppSettings` → `%APPDATA%\DiscordChatOverlay\settings.json`
...
### `ChannelContext` + `ChannelMember` → `%APPDATA%\DiscordChatOverlay\aliases.json`
...
### `TokenBundle` → Windows Credential Manager target `"DiscordChatOverlay"`
