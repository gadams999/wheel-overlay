# Feature Specification: Discord Chat Overlay

**Feature Branch**: `discord-chat-overlay/v0.1.0`
**Spec Folder**: `specs/004-discord-chat-overlay/`
**Created**: 2026-03-30
**Status**: Draft
...
A user opens the DiscordChatOverlay application and grants it permission to connect to their running Discord client. The application authenticates and begins receiving real-time voice activity data from Discord, without requiring the user to log in again after the initial setup.
...
1. **Given** a user has Discord running and launches DiscordChatOverlay for the first time, **When** they follow the authorization prompt, **Then** the application connects to Discord, persists the credentials, and shows a "Connected" status in the system tray icon or settings panel.
2. **Given** a user has previously authorized DiscordChatOverlay, **When** they launch the application, **Then** the application reconnects automatically without prompting for authorization again.
...
- **FR-007**: The system MUST persist user configuration (position, size, opacity, color theme) to a JSON settings file at `%APPDATA%\DiscordChatOverlay\settings.json`.
...
- **FR-009**: The system MUST log application events and errors to `%APPDATA%\DiscordChatOverlay\logs.txt` with 1 MB rotation, using OverlayCore's LogService.
- **FR-015**: The application MUST ship with a WiX 4 MSI installer, following the same structure and toolchain as WheelOverlay (`installers/discord-chat-overlay/`). The output MSI filename MUST include the application version number (e.g. `DiscordChatOverlay-v0.1.0.msi`).
...
- **FR-014c**: ChannelContext and ChannelMember data MUST be stored in a dedicated `aliases.json` file under `%APPDATA%\DiscordChatOverlay\`. Malformed or missing entries MUST be skipped and logged without crashing the application. The settings panel is the only supported interface for editing custom names and toggles.
...
- ChannelContext and ChannelMember records are managed exclusively through the DiscordChatOverlay settings panel; no import/export or sync mechanism is required in v0.1.0.
...
- Q: Does DiscordChatOverlay v0.1.0 require an installer? → A: Yes — WiX 4 MSI matching the WheelOverlay pattern (`installers/discord-chat-overlay/`). The output MSI filename MUST include the version number (e.g. `DiscordChatOverlay-v0.1.0.msi`). All new overlays follow this same installer process.
