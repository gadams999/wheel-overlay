...
**Owned by**: `DiscordIpcClient.cs` + `VoiceSessionService.cs` (`OpenDash.DiscordChatOverlay.Services`)

These are the Discord IPC events that `DiscordChatOverlay` subscribes to and the commands it sends. This document serves as the integration contract between `DiscordIpcClient` (transport) and `VoiceSessionService` (consumer).
