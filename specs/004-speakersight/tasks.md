...
- [x] T046 [P] Run `powershell -File scripts/Validate-PropertyTests.ps1 -TestProjectPath tests/SpeakerSight.Tests` from repo root — verify all 11 property tests have correct `#if FAST_TESTS / #else` iteration count guards and `// Feature: SpeakerSight, Property N: ...` comment directives; fix any directive or guard violations before proceeding
...
## Addendum: FR-016 / FR-016a (spec updated post-task-generation)

*Added from spec.md + plan.md updates: FR-016 (font-size-driven overlay dimensions) and FR-016a (settings preview mode). These were not present when the tasks above were originally generated.*

- [X] T049 [US3] Implement `MainWindow.RecomputeDimensions(int fontSize)` in `src/SpeakerSight/MainWindow.xaml.cs` — construct a `FormattedText` for the character `"W"` at `fontSize` using the overlay's `FontFamily`; `nameColumnWidth = 32 × formattedText.WidthIncludingTrailingWhitespace`; `windowWidth = 32 (avatar column) + columnGap + nameColumnWidth`; `windowHeight = 8 × (singleRowHeight + interRowSpacing)` where `singleRowHeight` is the `FormattedText.Height` at `fontSize`; set `this.Width` and `this.Height` to the computed values; call on `SourceInitialized` with `AppSettings.FontSize`; update `src/SpeakerSight/Settings/AppearanceSettingsCategory.cs` font-size slider `ValueChanged` handler to call `_mainWindow.RecomputeDimensions(newFontSize)` in addition to updating the `FontSize` resource (FR-016)
- [X] T050 [US3] Implement `OverlayViewModel.IsPreviewMode` in `src/SpeakerSight/ViewModels/OverlayViewModel.cs` — add `bool IsPreviewMode { get; set; }` with `INotifyPropertyChanged`; setter: when `true`, suppress all `VoiceSessionService` event handlers and repopulate `ActiveSpeakers` with 8 stub entries ("Speaker 1"–"Speaker 8"; first 5 `State=Active` `Opacity=1.0`, last 3 `State=RecentlyActive` `Opacity` 0.7/0.5/0.3); when `false`, clear stubs and re-populate from live `VoiceSessionService` state immediately; **update the already-committed `ShowSettings()` in `src/SpeakerSight/App.xaml.cs` (implemented in T031)** to also set `_overlayViewModel.IsPreviewMode = true` before showing `MaterialSettingsWindow` and `false` in the window `Closed` event handler — this is an additive change to existing T031 code (FR-016a)
